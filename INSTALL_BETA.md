...
6. 点击聊天输入框上方的“叙忆”按钮，或在扩展设置中打开“NarraMem 叙忆 0.4.0-beta.77”。记忆中心分为总览、记忆、当前任务、设置、日志与数据五页；手机上以底部面板显示。首次打开已有长聊天时，先核对待整理楼层和人工分批数量，再点击“开始整理旧聊天”；新聊天仍按自动 10＋4 运行。选择一个独立 API 配置：Custom 按酒馆自定义 Chat Completion 的习惯填写 URL 与 Key；DeepSeek 使用酒馆 1.18 原生 DeepSeek 适配，只填 Key。
